function getConfiguration(config)
{
  ...
  // address, you may want to use the code below.
    ...
    // config.addressLabel = {en: "Poke", es: "Poke"};
}
...
function getEndpoints(deviceAddress, endpoints)
{
  ...
  // temperature sensor, while the second one is a carbon dioxide sensor.
     ...
     endpoints.addEndpoint("3", "Temperature sensor", endpointType.VolumeSensor);
...
function validateDeviceAddress(address, result)
{
  ...
  // is exactly 10 characters long.
   ...
   if (address.length != 10) {
    result.ok = false;
     result.errorMessage = {
       en: "The address must be 6 characters long", 
      es: "La dirección debe tener exactamente 6 caracteres"
    };
   }
}
